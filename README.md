...
The playbook then configures OpenShift's apps domain to the cluster-scoped domain,
for example `apps.crc02.testing`, so routes such as the console and OAuth are
created under that host suffix.
...
- The API certificate is still generated by CRC and can show browser/CLI trust warnings. Routes are configured to the cluster-specific apps domain such as `apps.crc01.testing` or `apps.crc02.testing`.
